...
//        setContentView(R.layout.paypal_payment_activity);
    ...
    private PayPalPayment getStuffToBuy(String paymentIntent) {
        ...
        PayPalPayment payment = new PayPalPayment(amount, "USD", "Coupon Item", paymentIntent);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_PAYMENT) {
            if (resultCode == Activity.RESULT_OK) {
               ...
               // CommonUtils.showToast(mContext,"The user canceled.");
                Intent intent=new Intent(mContext,BuyCouponActivity.class);
                startActivity(intent);
            } else if (resultCode == PaymentActivity.RESULT_EXTRAS_INVALID) {
                //when amount is 0.0
                Log.i(TAG, "An invalid Payment or PayPalConfiguration was submitted. Please see the docs.");
                CommonUtils.showToast(mContext,"An invalid Payment or PayPalConfiguration was submitted. Please see the docs.");
            }
    ...
    private void checkOutApi(String tokenId, String userId, String couponId, String amount,String transactionId, String receiverID) {

        try {
            if (new ConnectionDetector(mContext).isConnectingToInternet()) {
                Hashtable<String, String> params = new Hashtable<>();
                JSONObject jObject = new JSONObject();
                jObject.put("method", "checkOut");
                jObject.put("userId", userId);
                jObject.put("couponId", couponId);
                jObject.put("transactionId", transactionId);
                jObject.put("amount", amount);
                jObject.put("tokenID", tokenId);
                jObject.put("receiverID", receiverID);
                params.put("json_data", jObject.toString());
                System.out.println("Request: " + params);
                webService = new CallWebService(mContext, new WebServiceApis(mContext).callApi(), params, Constants.SERVICE_TYPE.CHECKOUT, this);
                webService.execute();
            } else {
